...
#==========
#outpath: output path
#gwasfn: LDSC processed GWAS summary file 1 (e.g., vQTL GWAS results)
#mugwasfn: LDSC processed GWAS summary file 2 (e.g., alcohol cunsumption GWAS results)
#ldscpath: full path to wher LDSC is installed
#snplist: SNP list used for analysis (e.g., w_hm3.snplist)
#refldpath: reference LD score path (e.g., eur_w_ld_chr folder can be downloaded from LDSC website)

source activate ldsc
python /home/rx235/smtpred/ldsc_wrapper.py --out $outpath --files $gwasfn $mugwasfn  --ldscpath $ldscpath --snplist $snplist --ref_ld $refldpath --w_ld $refldpath

if [ $? -gt 0 ]; then
         echo "ERROR; failed to run Python script on the compute node"
         exit 1
       fi
